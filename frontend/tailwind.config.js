  ...
  theme: {
    extend: {
      ...
      colors: {
         ...
         *
         * NOTE: 这里必须为**每一家**建映射，缺一个就会静默少一类样式。
         * 之前的四个新托管商（dnspod / alidns / huaweicloud / vercel）在
         * index.css 里已定义变量、Badge.tsx 里也已引用类名，但漏了在这张表
         * 里登记 —— Tailwind 查不到颜色键就不生成对应类，于是那四家的
         * 来源徽章没有任何底色与字色，只有 ui-badge 的基础样式。
         * 这类错误不会报错、不会告警，只在界面上表现为「新家的徽章有点素」。
         * 排查方式：在产物 CSS 里搜 `.bg-source-<name>-bg` 是否存在。
         */
        source: {
          ...
          "dnspod-bg": "var(--source-dnspod-bg)",
          "dnspod-fg": "var(--source-dnspod-fg)",
          "alidns-bg": "var(--source-alidns-bg)",
          "alidns-fg": "var(--source-alidns-fg)",
          "huawei-bg": "var(--source-huawei-bg)",
          "huawei-fg": "var(--source-huawei-fg)",
          "vercel-bg": "var(--source-vercel-bg)",
          "vercel-fg": "var(--source-vercel-fg)"
        },
      ...
      /*
       * 层级令牌 —— 覆盖 Tailwind 默认的 z-0/10/20/30/40/50，新增语义四档，
       * 全部指向 index.css 的 --z-*。用法：z-dropdown / z-overlay / z-modal / z-toast。
       *
       * NOTE: 这里「只新增不删除」——存量代码仍有 z-10/z-20/z-50 等写法（如
       * 卡片内部相对定位的小徽章），删掉默认档会让它们静默失效。新增的语义档
       * 用于所有「页面级浮层」，两者并存不冲突（语义档数值 100+，永远高于默认档）。
       */
      zIndex: {
