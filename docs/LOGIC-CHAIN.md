...
**仍未解决的短板**：账号是**顺序遍历**，谁先耗尽谁触发 `break`，后面的账号整批跳过；下次 cron 又从**头**遍历，若第一个账号仍是大户，后面账号会**长期饿死**。

**两条未实施的方向**：
1. 账号遍历加「预算分配 + 轮转游标」→ 每个账号都能拿一份配额，无账号饿死
2. 拆多个 cron 表达式（免费计划最多 **3 个**）→ 拿到 3 份独立的 50 次配额
...
2. **[高]** 账号遍历加「预算分配 + 轮转游标」→ 彻底解决账号饿死
3. **[中]** 修正 `wrangler.toml` 与 README 里 cron 时区的说明（当前注释说"凌晨 2 点"，实际是北京时间 10:00）
4. **[中]** `schema.sql` 只有 7 张表，缺 `settings` 表（代码自举建表是权威，影响小但易误导）
5. **[低]** 自定义服务的三级结构定位未实现（跨源搜索跳转只切页）
6. **[低]** 拆分 `App.tsx`（13765 行单文件）
