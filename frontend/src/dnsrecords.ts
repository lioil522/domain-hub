...
export const COMMON_WITH_PTR_TYPE_OPTIONS: Array<{ value: string; label: string }> = [
  ...DNS_TYPE_OPTIONS,
  { value: "PTR", label: "PTR (反向解析)" },
];

/** 各服务商支持的记录类型选项映射 */
export const PROVIDER_DNS_TYPE_OPTIONS: Record<string, Array<{ value: string; label: string }>> = {
  cloudflare: CF_DNS_TYPE_OPTIONS,
  dnshe: DNS_TYPE_OPTIONS,
  digitalplat: DP_DNS_TYPE_OPTIONS,
  dnspod: DNS_TYPE_OPTIONS,
  alidns: COMMON_WITH_PTR_TYPE_OPTIONS,
  huaweicloud: COMMON_WITH_PTR_TYPE_OPTIONS,
  vercel: DNS_TYPE_OPTIONS,
};

/**
 * 根据服务商获取其支持的解析记录类型清单
 */
export function getDnsTypeOptionsForProvider(provider?: string): Array<{ value: string; label: string }> {
  const p = String(provider || "").toLowerCase();
  return PROVIDER_DNS_TYPE_OPTIONS[p] || CF_DNS_TYPE_OPTIONS;
}

/** 哪些服务商在架构上支持按线路分流解析 */
export const PROVIDER_SUPPORTS_LINE: ReadonlySet<string> = new Set([
  "dnspod",
  "alidns",
  "huaweicloud",
  "dnshe",
...
/**
 * 判断指定服务商（及域名）是否支持线路解析（Line）
 * - dnspod / alidns / huaweicloud：服务商原生支持线路解析，始终为 true
 * - dnshe：由域名的 NS 节点后缀是否命中支持名单决定（由 domainSupportsLine 传入）
 * - cloudflare / digitalplat / vercel 等：不支持分线路，返回 false
 */
export function providerSupportsLine(provider?: string, domainSupportsLine?: boolean): boolean {
  const p = String(provider || "").toLowerCase();
  if (p === "dnspod" || p === "alidns" || p === "huaweicloud") return true;
  if (p === "dnshe") return Boolean(domainSupportsLine);
  return false;
}
