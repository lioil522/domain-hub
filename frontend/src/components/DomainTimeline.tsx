...
export function DomainTimeline(props: DomainTimelineProps) {
  ...
  return (
    <section
      className={cn("bg-surface border border-border-base rounded-2xl overflow-hidden transition-all", className)}
    ...
    >
      ...
      <div className="px-4 sm:px-5 py-3.5 border-b border-border-base flex flex-wrap items-center gap-2">
        <CalendarClock className="w-4 h-4 text-accent shrink-0" aria-hidden="true" />
          ...
          <div className="px-3 sm:px-5 pt-4 pb-2">
            ...
            >
              {buckets.map((b) => {
                ...
                // 柱体颜色：深邃饱满的主题色调，彻底加深柱体填充度，消除空心泛白
                const barClass = count === 0
                  ? ""
                  : b.isCurrent
                    ? "bg-accent-gradient shadow-[0_0_16px_rgb(var(--accent-rgb)/0.6)] border border-accent ring-1 ring-accent/60"
                    : isOpen
                      ? "bg-accent border-2 border-white/80 shadow-[0_0_18px_rgb(var(--accent-rgb)/0.7)]"
                      : "bg-accent/80 hover:bg-accent border border-accent/60 shadow-sm transition-all";
                      ...
                      {count === 0 ? (
                        ...
                        <span
                          className={cn(
                            ...
                            barClass,
                            isOpen ? "ring-2 ring-accent shadow-[0_0_14px_rgb(var(--accent-rgb)/0.4)]" : "group-hover:brightness-110"
                          )}
            ...
            {/* 当前月标记说明 */}
            <div className="mt-2 px-1 text-[11px] text-content-muted flex items-center gap-1.5">
              <span className="inline-block w-2.5 h-2.5 rounded-sm bg-accent-gradient shrink-0" aria-hidden="true" />
              高亮柱为当前月份，点击柱体可展开当月到期域名
            </div>
          </div>

          {/* 展开区：选中月份的域名清单 */}
          {openBucket && openBucket.items.length > 0 && (
            <div className="border-t border-border-base bg-elevated/20">
              <div className="px-4 sm:px-5 py-2.5 bg-hovered/50 border-b border-border-soft flex items-center gap-2">
