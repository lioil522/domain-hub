...
export class DatabaseManager {
  ...
  /** 新绑定 / 换 Key：拉一次该账号的配额写回缓存，只影响这一个账号（Cloudflare / DigitalPlat 账号无 DNSHE 式配额概念，直接清掉缓存条目） */
  async refreshAccountQuotaCache(accountId: number, alias: string, provider: AccountProvider = "dnshe"): Promise<void> {
    const cached = await this.readQuotaCache();
    if (cached === null) return;

    if (provider === "cloudflare" || provider === "digitalplat" || provider === "custom") {
      await this.writeQuotaCache(cached.filter((q) => Number(q.account_id) !== accountId));
      return;
    }
  ...
  /**
   * 根据 ID 获取解密后的 API 客户端（按账号 provider 返回 DNSHE / Cloudflare / DigitalPlat 客户端）
   */
  async getClientForAccount(id: number): Promise<{ client: DNSHEClient | CloudflareClient | DigitalPlatClient; alias: string; provider: AccountProvider }> {
  ...
  async getDomains(
    ...
    if (provider === "cloudflare") {
      ...
      query += " AND IFNULL(a.provider, 'dnshe') NOT IN ('cloudflare', 'digitalplat', 'custom')";
    } else {
      query += " AND IFNULL(a.provider, 'dnshe') NOT IN ('cloudflare', 'digitalplat', 'custom')";
    }
