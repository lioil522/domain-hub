...
export function useAppControllerView() {
  ...
  // 进入「注册/查重」「账户配额」「解析线路」页时自动展开 DNSHE 子菜单（否则用户通过
  // hash 直入 #/register / #/quota / #/line-settings 时，侧栏 DNSHE 项虽高亮但子菜单收起，
  // 看不到子项被选中）
  useEffect(() => {
    if (activeTab === "register" || activeTab === "quota" || activeTab === "line-settings") setDnsheMenuOpen(true);
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    refreshAllProviderDomains();
    // DigitalPlat / DNSPod / 阿里云 / 华为云 / Vercel 域名同理：独立标签页数据 +
    // 徽标计数，随会话一次拉齐
   ...
   * "Cloudflare" / "DigitalPlat" / "DNSPod" / "阿里云 DNS" / "华为云 DNS" /
   * "Vercel" / "自定义"），而跳转分派用的是短键（"dnshe" / "cf" / "dp" /
   * MultiProviderKey / "custom"）。中间需要一层显式映射，不能靠字符串相等
   * —— "Cloudflare" 不等于 "cf"，"阿里云 DNS" 也不等于 "alidns"。
   *
   * 匹配用 includes 而非 === ：显示名可能带后缀（如 "阿里云 DNS"、"华为云
   * 云解析"），前缀/子串匹配比全等更耐改。四个新托管商的判定必须放在最后的
   * 兜底 return 之前。
   *
  ...
  /**
   * 跨来源搜索跳转 / 时间轴定位：根据来源与完整域名切到对应标签页。
   *
   * WHY 需要这个函数：跨源搜索的命中项来自 8 个不同的数据源，每个源在
   * 页面上是独立的标签页 + 独立的过滤状态。统一在这里做「切页 + 清搜索」
   * 两件事，避免每个调用点各写一遍。
   *
   * DNSHE/CF/DP 有三级结构（账号 → 域名 → 记录），复用已有的 gotoXxx
   * 定位到具体域名；四家新托管商与自定义服务商目前只切页 —— 它们的页面
   * 同样有账号分组，但定位高亮待页面渲染落地后补。
   *
   * 跳转后清空全局搜索是刻意的：搜索词会继续过滤目标页的列表，导致用户
   * 点了命中项却在目标页「看不到」那条数据。
   */
